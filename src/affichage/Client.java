...
public class Client extends TypeUtilisateur {
	...
	private void supprimerImpression() {
		...
		/*Requ�te des impressions appartenant au client*/
			//EN ATTENDANT
		...
		while(!flagUp) {
			...
			if(reponse>1 && reponse<=impressions.size()) {
				...
				if(choix.equals("V")) {
					...
					System.out.println("Le fichier Image : "+impToDelete.getPathImpression()+" � �t� supprim�.");
	...
	private boolean creerAgendaSemaines(boolean isInModifMode) {
		...
		if(isInModifMode){
			while(!flagUp) {
				...
				/*Requ�te de tous les agenda semaines du client*/
			...
			System.out.println("/*************** Cr�ation d'un Agenda Semaine **************/\n\n");
	...
	private boolean enregistrerSemaine(String path, ArrayList<Couple<Photo>> photos) {
		...
		boolean flag = false;
		while(!flag) {
			System.out.println("En quelle qualit� souhaitez-vous cr�er votre Agenda Semaine?\n"
					+ 	"2 - Moyenne\n"
					+ 	"1 - Sup�rieur\n"
		...
		flag = false;
		while(!flag) {
			System.out.println("En quel format souhaitez-vous cr�er votre Agenda Semaine?\n"
					+ 	"2 - A4\n"
					+ 	"1 - A5\n"
					+ 	"0 - Annuler l'enregistrement\n");
			reponse = LectureClavier.lireEntier("\nChoix :");
		...
		if(semaineControler.create(toCreate)) {
			System.out.println("Agenda Semaine enregistr�. \n");
			return true;
		}else{
			System.out.println("Attention cet Agenda Semaine existe d�j�, veuillez modifier le chemin.\n");
		}
		return false;
	}
	...
	private boolean enregistrerJours(String path, ArrayList<Couple<Photo>> photos) {
		...
		boolean flag = false;
		while(!flag) {
			System.out.println("En quelle qualit� souhaitez-vous cr�er votre Agenda Jour?\n"
					+ 	"2 - Moyenne\n"
					+ 	"1 - Sup�rieur\n"
					+ 	"0 - Annuler l'enregistrement\n");
			reponse = LectureClavier.lireEntier("\nChoix :");
		...
		flag = false;
		while(!flag) {
			System.out.println("En quel format souhaitez-vous cr�er votre Agenda Jour?\n"
					+ 	"2 - A4\n"
					+ 	"1 - A5\n"
					+ 	"0 - Annuler l'enregistrement\n");
			reponse = LectureClavier.lireEntier("\nChoix :");
	...
	private boolean creerCalendrierBureau(boolean isInModifMode) {
		...
		if(isInModifMode){
			while(!flagUp) {
				...
				/*Requ�te de tous les Calendrier Bureau du client*/
			...
			System.out.println("/************** Cr�ation d'un calendrier Bureau *************/\n\n");
	...
	private boolean enregistrerBureau(String path, ArrayList<Couple<Photo>> photos) {
		...
		boolean flag = false;
		while(!flag) {
			System.out.println("En quelle qualit� souhaitez-vous cr�er votre calendrier Bureau?\n"
					+ 	"2 - Moyenne\n"
					+ 	"1 - Sup�rieur\n"
					+ 	"0 - Annuler l'enregistrement\n");
			reponse = LectureClavier.lireEntier("\nChoix :");
		...
		flag = false;
		while(!flag) {
			System.out.println("En quel format souhaitez-vous cr�er votre calendrier Bureau?\n"
					+ 	"2 - A4\n"
					+ 	"1 - A5\n"
					+ 	"0 - Annuler l'enregistrement\n");
			reponse = LectureClavier.lireEntier("\nChoix :");
		...
		if(bureauInterface.create(toCreate)) {
			System.out.println("calendrier Bureau enregistr�. \n");
			return true;
		}else
		{
			System.out.println("Attention ce calendrier Bureau existe d�j�, veuillez modifier le chemin.\n");
		}
		return false;
	}
	...
	private boolean creerCalendrierMuraux(boolean isInModifMode) {
		...
		if(isInModifMode){
			while(!flagUp) {
				...
				/*Requ�te de tous les calendrier mural du client*/
	...
	private boolean creerAlbum(boolean isInModifMode) {
		...
		if(isInModifMode){
			while(!flagUp) {
				...
				/*Requ�te de tous les albums du client*/
			...
			if(photos.size()==0) {
				...
				switch(reponse) 
				{
					case 5 : if(!isInModifMode) {
								...
								System.out.println("Tirage modifi�.\n");
								return true;
							}
	...
	private ArrayList<Couple<Photo>> AjouterPhotoAPage(int nbMaxPage, ArrayList<Couple<Photo>> photos, int id, String questionNumero) {
		int idphoto = id;
		/*Requ�te pour v�rifier si la photo appartient au client ou qu'il peut utiliser une photo partag�e*/
			//EN ATTENDANT

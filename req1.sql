...
CREATE OR REPLACE TRIGGER NB_PAGE_AGENDA
BEFORE INSERT OR UPDATE ON PHOTOAGENDA
FOR EACH ROW
DECLARE 
   NBRES INTEGER;
   NO_PAGE INTEGER;
...
BEGIN
   ...
   FROM SEMAINES
   WHERE NUMIMPRESSION = :NEW.NUMIMPRESSION;
   ...
   IF( TYPE = 'JOURS' AND :NEW.NOPAGEPHOTOAGENDA >= 365 ) THEN 
      RAISE_APPLICATION_ERROR(-20001, 'NO_PAGE INVALIDE');
   ELSE
      IF( TYPE = 'SEMAINES' AND :NEW.NOPAGEPHOTOAGENDA >= 52 ) THEN 
         RAISE_APPLICATION_ERROR(-20001, 'NO_PAGE INVALIDE');
      END IF;
   END IF;
END;
